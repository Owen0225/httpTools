...
if ($Target -in @("release", "all")) {
    ...
    $releaseCmd = @"
rc /nologo /I "smtpToolApp" /I "assets" /fo "build\ci\obj\release\version.res" "smtpToolApp\version.rc" && cl /nologo /std:c++20 /O2 /GL /Gy /EHsc /MT /DNDEBUG /DUNICODE /D_UNICODE /DWIN32 /D_WINDOWS /I "$curlInclude" /I "smtpToolCore" /I "smtpToolApp" "smtpToolCore\RawRequestParser.cpp" "smtpToolCore\CurlCommandParser.cpp" "smtpToolCore\CppCodeGenerator.cpp" "smtpToolCore\HttpExecutor.cpp" "smtpToolApp\AppMain.cpp" "smtpToolApp\MainWindow.cpp" "build\ci\obj\release\version.res" /Fe:"build\ci\release\smtpTool.exe" /link /LTCG /OPT:REF /OPT:ICF /INCREMENTAL:NO /RELEASE /DYNAMICBASE /NXCOMPAT /MANIFEST:EMBED /MANIFESTINPUT:"smtpToolApp\app.manifest" /LIBPATH:"$curlLibDir" libcurl.lib Comctl32.lib Comdlg32.lib User32.lib Gdi32.lib Ws2_32.lib Crypt32.lib Wldap32.lib Normaliz.lib
"@
    cmd /c $releaseCmd
    if ($LASTEXITCODE -ne 0) {
