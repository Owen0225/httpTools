...
if ($Target -in @("release", "all")) {
    ...
    $releaseCmd = @"
"$vcvars" && rc /nologo /I "smtpToolApp" /I "assets" /fo "build\obj\release\version.res" "smtpToolApp\version.rc" && cl /nologo /std:c++20 /O2 /GL /Gy /EHsc /MT /DNDEBUG /DUNICODE /D_UNICODE /DWIN32 /D_WINDOWS /I "D:\C2\curl\include" /I "smtpToolCore" /I "smtpToolApp" "smtpToolCore\RawRequestParser.cpp" "smtpToolCore\CurlCommandParser.cpp" "smtpToolCore\CppCodeGenerator.cpp" "smtpToolCore\HttpExecutor.cpp" "smtpToolApp\AppMain.cpp" "smtpToolApp\MainWindow.cpp" "build\obj\release\version.res" /Fe:"build\release\smtpTool.exe" /link /LTCG /OPT:REF /OPT:ICF /INCREMENTAL:NO /RELEASE /DYNAMICBASE /NXCOMPAT /MANIFEST:NO /LIBPATH:"D:\C2\curl\build_rel\lib" libcurl_imp.lib Comctl32.lib Comdlg32.lib User32.lib Gdi32.lib && mt.exe -manifest "smtpToolApp\app.manifest" -outputresource:"build\release\smtpTool.exe;#1"
"@
    cmd /c $releaseCmd
    if ($LASTEXITCODE -ne 0) {
